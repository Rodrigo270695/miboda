...
function DetailCard({
  ...
  return (
    <div className="rounded-3xl border border-[rgba(212,175,55,0.4)] bg-white/88 px-6 py-7 shadow-[0_18px_45px_rgba(90,33,69,0.12)]">
  ...
  return (
    <section id="detalles" className="px-10 py-16 text-center">
      <div className="w-full rounded-3xl border border-[rgba(212,175,55,0.4)] bg-white/88 px-5 py-7 shadow-[0_18px_45px_rgba(90,33,69,0.12)]">
        <SectionTitle subtitle="Una aventura llena de color te espera">
          Detalles
        </SectionTitle>
        <div className="grid gap-6">
          <DetailCard title="Fecha y hora">
            <p>{wedding.event.dateLabel}</p>
            {wedding.event.timeLabel ? <p>{wedding.event.timeLabel}</p> : null}
          </DetailCard>
          <DetailCard title="Lugar">
            <p>{wedding.event.locationName}</p>
            {wedding.event.mapUrl ? (
              <a
                className="mt-5 inline-flex rounded-full border border-[var(--blue-soft)] px-6 py-2.5 text-sm uppercase tracking-[0.22em] text-[var(--fuchsia)] transition hover:bg-[var(--fuchsia)] hover:text-white"
